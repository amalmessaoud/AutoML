...
from src.config.constants import IMPROVEMENT_MARGIN, ABSOLUTE_FLOOR, is_solved, compute_dummy_score
...
class TestCritiqueDeterminism:
    ...
    def test_dummy_score_returned_in_result(self):
        ...
        with patch.object(GROQ_LLAMA_8B, "build_client", return_value=_mock_llm('{"reason": "ok", "suggestion": ""}')):
            agent = CritiqueAgent(config=GROQ_LLAMA_8B, logs=[])
            result = agent.run(plan=plan, results=results, problem="test", X=X, y=y)
