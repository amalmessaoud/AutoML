...
class AutoMLPlan(BaseModel):
    ...
    primary_metric: Literal["accuracy", "f1_macro", "f1_weighted", "roc_auc", "balanced_accuracy"] = Field(..., description="Primary metric for model evaluation")
    preprocessing_steps: List[PreprocessingStep] = Field(
        default_factory=list,
        description="List of preprocessing steps (can be empty)"
    )
    models_to_try: List[ModelToTry] = Field(..., description="List of models to try during training")
    validation_method: Literal["cross_validation"] = Field(
        "cross_validation",
        description="Validation strategy - fixed to cross-validation"
    )
