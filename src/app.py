...
with gr.Blocks(title='Structured LLM-Agent AutoML') as demo:
    ...
    gr.Markdown(
        'Upload a tabular dataset and describe the prediction task. The agents will autonomously plan, execute, and improve until solved.'
    )
    ...
    run_btn.click(
        fn=automl_interface, inputs=[csv_input, problem_input], outputs=[summary_output, log_output]
    )
